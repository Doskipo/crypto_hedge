...
S0 = df.iloc[0]['price']
STRIKE_1 = S0              # Venem aquesta (ATM)
STRIKE_2 = S0 * 1.05      # Comprem aquesta (5% OTM) per cobrir Gamma
# T_TOTAL_DIES = (df['timestamp'].max() - df['timestamp'].min()).total_seconds() / (24 * 3600)
T_TOTAL_DIES = 30.0
T_TOTAL_ANYS = T_TOTAL_DIES / 365.0
R = 0.0

FREQ_HORES = 24 # Freqüència de rebalanç (1 cop al dia)
FEE = 0    # 3 bps per a Deribit (tant spot com opcions)
...
print(f"- Rebalanç cada {FREQ_HORES} hores.\n")
...
for i in range(len(df) - 1):
    ...
    iv_actual = df.iloc[i]['iv'] # La volatilitat real de Deribit a cada moment!
    ...
    if i % FREQ_HORES == 0:
        ...
        q_op2_nova = gamma_1 / gamma_2 if gamma_2 > 0 else 0.0
...
# Retallem el temps per quadrar amb la llista d'errors (evitant l'últim punt trencat)
ax2.plot(llista_temps, llista_errors, color='purple', linewidth=1.5, label='Error de Cobertura Delta-Gamma')
ax2.axhline(y=0, color='black', linewidth=1)
ax2.set_title(f'Evolució de l\'Error (Rebalanç cada {FREQ_HORES} hores)')
ax2.set_xlabel('Data')
